...
def guardar_operacion(simbolo, margen, apalancamiento, tipo_operacion, resultado, pnl, fecha_iso, duracion):
    try:
        ...
        sql_insert = """
            INSERT INTO operaciones (simbolo, margen, apalancamiento, tipo_operacion, resultado, pnl, fecha, duracion)
...
def obtener_balance_disponible():
    """
    Obtiene el balance disponible en la cuenta de futuros.
    """
    try:
...
def crear_orden(simbolo, porcentaje_cuenta, apalancamiento, direccion):
    ...
    try:
        ...
        valor_nocional = cantidad_ajustada * precio_actual
...
def cerrar_operacion(simbolo, cantidad, cantidad_usdt, direccion, estado, resultado, pnl_final, timestamp_inicio):
    try:
        ...
        if estado == "SL" and pnl_final < 0:
            mensaje = f"🔴 SL alcanzado. PNL Final: {pnl_final:.2f} USDT - ROI: {resultado:.2f}% - Mercado: {simbolo} - {minutos}m {segundos}s"
            ...
            mensaje = f"🎉Profit protegido alcanzado🎉               PNL: {pnl_final:.2f} USDT  -  ROI: {resultado:.2f}% - {minutos}m {segundos}s"
            print(f"La operación estuvo abierta durante {minutos} minutos y {segundos} segundos.")
            enviar_notificacion_telegram(mensaje, tipo="positivo")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if orden_inicial:
            ...
            if estado == "SL" or estado == "None":
                tiempo_fin = time.time()  # Registrar tiempo de cierre
